...
def prepare_world_rl_data(json_path: str) -> List[Dict[str, Any]]:
    """
    ...
    for entry in raw_data:
        ...
        if entry.get('human_or_machine', 0) == 0:
            ...
            new_entry['direct_answers'] = eval(new_entry['direct_answers'])
            image = new_entry.get('image', '')
            if not os.path.exists(image):
                image = image.replace('/chartqa_output/',
                                      '/path/to/data/chartqa_output/')
...
def prepare_world_dyme_data(json_path: str) -> List[Dict[str, Any]]:
    """
    Processes a JSON file of chart data for Reinforcement Learning.
    ...
    for entry in raw_data:
        ...
        if entry.get('human_or_machine', 0) == 0:
            ...
            new_entry.pop('human_or_machine', None)
            new_entry.pop('question', None)
            new_entry['hint'] = entry['visual_fact']
...
def prepare_world_sft_data(json_path: str) -> List[Dict[str, Any]]:
    """
    Processes a JSON file of chart data for Reinforcement Learning.
