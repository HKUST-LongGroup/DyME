...
from client_eval import get_smol_reason_reward, get_smol_acc_reward, get_smol_expert_reward, get_mllm_reward, \
    get_format_reward, get_other_express
# os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
from datasets import Features, Image, Value, Dataset, load_dataset
from my_grpo_good_vlm_llava import MyGRPOTrainer
...
# parser.add_argument('--task', type=str, default='medical', help='task')
...
def collate_fn(examples):
    ...
    labels[labels == 151646] = -100
    batch["labels"] = labels

    return batch
# from transformers import Idefics3Processor, Idefics3ImageProcessor
...
def deal_llava_med_slake(max_completion_length):
    ...
    for dd in tqdm(ds):
        ...
        question_with_tags = mpo_prompt_with_final_answer.format(question=question)
...
def deal_chart_rl_data(max_completion_length):
    ...
    json1 = '/home/jliugj/HDD/academic/database/chart/chartQA/accept_train_mp_hint.json'
    need_change_path = False
    new_dir = None
    if not os.path.exists(json1):
        new_dir = '/project/longgroup/jiazhen/dataset/moe_efficient/chartQA'
        json1 = os.path.join(new_dir, 'accept_train_mp_hint.json')
    ...
    for i in range(len(data)):
        ...
        data[i]['prompt'] = question_with_tags
        name = os.path.basename(data[i]['image'])
        # data[i]['image'] = os.path.join('/export/jliugj/academic/database/chart/chartQA/images/', name)
        if need_change_path:
            image_name = os.path.basename(data[i]['image'])
            data[i]['image'] = os.path.join(new_dir, 'images', image_name)

        data[i]['knowledge'] = data[i]['hint']
    return data
...
outpit_pos = '/home/jliugj/HDD/academic/checkpoints/llava-ov_abl'
need_path_change = False
if not os.path.exists(outpit_pos):
    outpit_pos = 'checkpoints/'
    need_path_change = True
model = None

raw_model_id = "llava-hf/llava-onevision-qwen2-0.5b-ov-hf"
raw_model_id = "llava-hf/llava-onevision-qwen2-0.5b-si-hf"
...
if is_sft:
    ...
    # 模型与处理器初始化
    if task is not None:
        output_dir = os.path.join(outpit_pos, task + "_long_answer_base")
    ...
    deal_data = define_task_data_func(task)
    output_dir = os.path.join(outpit_pos, task + "_long_answer_instruct")
    sft_args = GRPOConfig(
        output_dir=output_dir,
        logging_steps=1,
        num_generations=1,
...
task = tasks[2]
output_dir = os.path.join(outpit_pos, task + "llava_abl_long_answer_rl-512")
max_completion_length = 144
...
trainer.train(resume_from_checkpoint='/home/jliugj/HDD/code/moe_efficient/checkpoints/chart_rlllava_abl_long_answer_rl-512/checkpoint-700/')
